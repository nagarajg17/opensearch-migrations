...
// These produce BareQueryNode. The parser.ts wrapper sets the defaultField
// property based on the df parameter.

// BareQueryNode (bare phrase): "hello world" without a field prefix.
// `"hello world"` → BareQueryNode { query: "hello world", isPhrase: true }
// defaultField is set by parser.ts based on df parameter.
barePhrase
  = "\"" text:$[^"]* "\"" boost:boost? {
      const node = { type: 'bareQuery', query: text, isPhrase: true };
      if (boost !== null) return { type: 'boost', child: node, value: boost };
      return node;
    }

// BareQueryNode (bare term): a search term without a field prefix.
// `java` → BareQueryNode { query: "java", isPhrase: false }
// defaultField is set by parser.ts based on df parameter.
...
bareValue
  = val:valueChars boost:boost? {
      ...
      const node = { type: 'bareQuery', query: val, isPhrase: false };
      if (boost !== null) return { type: 'boost', child: node, value: boost };
      return node;
    }
