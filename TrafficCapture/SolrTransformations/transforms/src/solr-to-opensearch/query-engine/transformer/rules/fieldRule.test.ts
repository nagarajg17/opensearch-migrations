...
describe('fieldRule', () => {
  ...
  it.each([
    ...
    expect(result).toEqual(
      new Map([['term', new Map([['status', 'active']])]]),
    );
  });
  ...
  it('transforms fuzzy search with distance to fuzzy query with fuzziness', () => {
    ...
    const params = fuzzyMap.get('title') as Map<string, any>;
    expect(params.get('value')).toBe('roam');
    expect(params.get('fuzziness')).toBe(2);
  });
  ...
  it('transforms fuzzy search with distance 1', () => {
    ...
    const params = fuzzyMap.get('title') as Map<string, any>;
    expect(params.get('value')).toBe('roam');
    expect(params.get('fuzziness')).toBe(1);
  });
  ...
  it('transforms fuzzy search with distance 0', () => {
    ...
    const params = fuzzyMap.get('title') as Map<string, any>;
    expect(params.get('value')).toBe('roam');
    expect(params.get('fuzziness')).toBe(0);
  });
  ...
  it('term query is used for exact field inside a bool query via translateQ', () => {
    const fieldTypes = new Map([['status', 'solr.StrField']]);
    const params = new Map([['q', 'status:active']]);

    const { dsl } = translateQ(params, 'fail-fast', fieldTypes);

    expect(dsl).toEqual(new Map([['term', new Map([['status', 'active']])]]));
  });
  ...
  it('term query propagates through AND bool node', () => {
    ...
    // Both fields are exact → both should be term queries inside bool.must
    const boolMap = dsl.get('bool') as Map<string, any>;
    const must = boolMap.get('must') as Map<string, any>[];
    expect(must[0]).toEqual(new Map([['term', new Map([['status', 'active']])]]));
    expect(must[1]).toEqual(new Map([['term', new Map([['category', 'books']])]]));
  });
});